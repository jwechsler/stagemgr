FactoryBot.define do
  ...
  factory :ticket_order do
    ...
    after(:create) do |ticket_order, _evaluator|
      if ticket_order.performance.production.has_reserved_seating?
        ...
        ticket_order.flatten_ticket_line_items.select { |t| t[:seat].nil? }.each do |_flattened_item|
          next unless seats_required > 0
    ...
    trait :paid_with_flex_pass do
      ...
      before(:create) do |ticket_order, evaluator|
        ...
        ticket_order.performance.ticket_class_allocations << FactoryBot.build(:ticket_class_allocation,
                                                                              available: true, ticket_class: pass_class)
      ...
      after(:create) do |ticket_order, _evaluator|
        ...
        find_code = flex_pass.flex_pass_offer.use_ticket_class_code
        new_ticket_class = ticket_order.performance.ticket_class_allocations.select do |tca|
          tca.ticket_class.class_code.eql?(find_code)
        end.first.ticket_class
        ticket_order.ticket_line_items.each do |tli|
        ...
        if !ticket_order.service_line_items.empty?
          ticket_order.payments << FactoryBot.create(:cash_payment,
                                                     order: ticket_order,
                                                     number_of_tickets: num_tix,
                                                     amount: ticket_order.service_line_items.inject(0) do |total, sli|
                                                       total + sli.amount
                                                     end)
        end
        ticket_order.status = Order::PROCESSED
    ...
    trait :paid_with_membership do
      ...
      before(:create) do |ticket_order, evaluator|
        membership = if evaluator.member_code.blank?
                       FactoryBot.create(:membership, address: ticket_order.address)
                     else
                       FactoryBot.create(:membership, address: ticket_order.address, member_code: evaluator.member_code)
                     end
      ...
      after(:create) do |ticket_order, _evaluator|
        ...
        new_ticket_class = ticket_order.performance.ticket_class_allocations.select do |tca|
          tca.ticket_class.class_code == find_code
        end.first.ticket_class
        ticket_order.ticket_line_items.each do |tli|
          tli.ticket_class = new_ticket_class
        end
  ...
  trait :one_thousand_dollar_donation do
    association :payment_type, factory: :credit_card_payment_type
    after(:create) do |donation_order, _evaluator|
  ...
  factory :donation_pledge_order do
    ...
    factory :donation_pledge_order_for_one_thousand_dollars_using_credit_card,
            traits: %i[one_thousand_dollar_donation paid_with_credit_card]
  end
