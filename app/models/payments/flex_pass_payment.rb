class FlexPassPayment < PassPayment
  ...
  validates_each :number_of_tickets do |record, attr, value|
    ...
    if record.number_of_tickets > 0 then
      ...
      record.errors.add(attr,
                        "cannot be more than the number of tickets left on flex pass.") if number_of_tickets_left_after_save < 0
      ...
      unless offer.maximum_uses_per_production.nil? || offer.maximum_uses_per_production.eql?(0)
        ...
        record.errors.add(attr,
                          " has been exceeded for #{record.order.performance.production.name}.  This flex pass can only be redeemed for #{offer.maximum_uses_per_production} ticket(s)/production, #{record.number_of_tickets + already} requested") if (already + record.number_of_tickets) > offer.maximum_uses_per_production
      end
    end
  end
  ...
  def process!(order = nil)
    ...
    tc_list = order.performance.ticket_class_allocations.select { |tca|
      tca.available
    }.map { |tca| tca.ticket_class.class_code }
