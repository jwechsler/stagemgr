class Membership < ActiveRecord::Base
  ...
  def verify_applicable_for(order)
    ...
    prods = Production.count(:conditions=>["exists (select * from performances, orders, payments where payments.type = 'MembershipPayment' and payments.order_id = orders.id and orders.performance_id = performances.id and performances.production_id = productions.id and payments.membership_id = ? and orders.id != ? and productions.id = ? and orders.status in (?))", self.id, order.id, order.performance.production_id, Order.attending_statuses])
