class FlexPass < ApplicationRecord
  ...
  def create_code(size = 6)
    ...
    while self.code.nil? || !FlexPass.find_by_code(self.code).nil?
      self.code = (self.flex_pass_offer.code_prefix.blank? ? "" : self.flex_pass_offer.code_prefix) + (0...size).map {
  ...
  #
  def ticket_orders
    orders = TicketOrder.joins(:payments).where("payments.type = 'FlexPassPayment' and flex_pass_id = :fp_id",
                                                { fp_id: self.id })
  end
