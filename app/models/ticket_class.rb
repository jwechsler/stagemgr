class TicketClass < ApplicationRecord
  ...
  def prevent_price_changes_after_sales
    unless self.ticket_type == DONATION
      if (self.ticket_price_was != self.ticket_price) && TicketLineItem.where(ticket_class_id: self.id).size > 0
        errors.add(:base,
                   "Cannot change ticket price from #{self.ticket_price_was} to #{self.ticket_price} if sales have already occurred")
        return false
      end
    end
    return true;
  ...
  def number_taken(performance, exclude_order = nil)
    if exclude_order.nil?
      TicketLineItem.where(
        "ticket_class_id = :tc_id and exists (select * from orders where orders.id = order_id and performance_id = :performance_id)", tc_id: self.id, performance_id: performance.id
      ).sum(:ticket_count)
    else
      TicketLineItem.where(
        'ticket_class_id = :tc_id and exists (select * from orders where orders.id = order_id and performance_id = :performance_id) and order_id != :order_id', tc_id: self.id, performance_id: performance.id, order_id: exclude_order.id
      ).sum(:ticket_count)
    end
  end
  ...
  def check_for_shift_to_codes
    return true if TicketClassAllocation.joins(:performance).where(
      "performances.production_id = :prod_id and shift_to_code = :shift_to", prod_id: self.production_id, shift_to: self.class_code
    ).count > 0
