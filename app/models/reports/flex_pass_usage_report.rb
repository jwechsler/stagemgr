class FlexPassUsageReport < Report
  attr_reader :starting_date, :ending_date, :flex_pass_offer_id

  def initialize(starting_date, ending_date, flex_pass_offer_id = nil, reporting_user_id = nil)
    super(%i[month new_passes new_deposits tickets_paid_out total_spiff
             total_flat_payout total_facility expired_flex_passes
             recovered_amount total_due_to_facility], reporting_user_id)
    @starting_date = starting_date
    @ending_date = ending_date
    @flex_pass_offer_id = flex_pass_offer_id
    @data = []
  end
  ...
  def create
    ...
    paid_amount = FlexPassPayment.where('processed_on >= :start and processed_on <= :end',
                                        start: starting_date, end: ending_date)
    if flex_pass_offer_id.present?
      paid_amount = paid_amount.joins(flex_pass: :flex_pass_offer).where(
        flex_passes: { flex_pass_offer_id: flex_pass_offer_id }
      )
    end
    ...
    if flex_pass_offer_id.present?
      collected_amounts = collected_amounts.where(flex_passes: { flex_pass_offer_id: flex_pass_offer_id })
    end
    ...
    recovered_amounts = FlexPass.joins(:flex_pass_payments).joins(:flex_pass_offer)
                                .where('expiration_date >= :starting_date AND expiration_date <= :ending_date',
                                       starting_date: starting_date, ending_date: ending_date)
    if flex_pass_offer_id.present?
      recovered_amounts = recovered_amounts.where(flex_pass_offers: { id: flex_pass_offer_id })
    end
    recovered_amounts = recovered_amounts.group("DATE_FORMAT(expiration_date, '%Y-%m')").select(
