class HistoricActivityReport < MailingList
  ...
  def initialize(start_day, minimum_attended = 0, required_theater_ids = [], minimum_revenue = 0.0,
    ...
    # build headers
    headers = TRG_IMPORT_HEADERS + [:PrimaryTheatreAttendee, :LastAttended, :AttendedInPeriod, :TotalAttended,
  ...
  def create
    ...
    addresses = orders.map { |o|
      o.address
    }.uniq.select { |a| !a.nil? && a.productions_attended(start_day).size >= minimum_attended && a.revenue_collected(start_day) >= minimum_revenue }.sort { |a, b| a.last_name <=> b.last_name }
    ...
    addresses.each do |address|
      ...
      prods = requested_prods.sort { |a, b|
        if b.opening_at.nil?
          false
      ...
      self.data['ALL'] << MailingList.mailing_hash_from_buyer(address).merge({ :PrimaryTheatreAttendee => primary_attendee ? "*" : "",
                                                                               ...
                                                                               :CompaniesAttendedInPeriod => requested_prods.map { |p|
                                                                                 p.theater_id
                                                                               }.uniq.size,
                                                                               :TotalCompaniesAttended => all_prods.map { |p|
                                                                                 p.theater_id
                                                                               }.uniq.size,
