class HistoricActivityReport < MailingList
  attr_accessor :required_theater_ids, :minimum_attended, :minimum_revenue, :start_day

  def initialize(start_day, minimum_attended = 0, required_theater_ids = [], minimum_revenue = 0.0,
    ...
    # build headers
    headers = TRG_IMPORT_HEADERS + %i[PrimaryTheatreAttendee LastAttended AttendedInPeriod TotalAttended
                                      CompaniesAttendedInPeriod TotalCompaniesAttended IsMember IsFlexPassHolder ProductionHistory]
  ...
  def create
    ...
    addresses = orders.map do |o|
      o.address
    end.uniq.select { |a| !a.nil? && a.productions_attended(start_day).size >= minimum_attended && a.revenue_collected(start_day) >= minimum_revenue }.sort { |a, b| a.last_name <=> b.last_name }
    ...
    addresses.each do |address|
      ...
      prods = requested_prods.sort do |a, b|
        if b.opening_at.nil?
          false
      ...
      end.map { |p| "#{p.name} [#{p.theater.name}]" }
      prodlist = prods.join(', ')
      data['ALL'] << MailingList.mailing_hash_from_buyer(address).merge({ PrimaryTheatreAttendee: primary_attendee ? '*' : '',
                                                                          FullName: address.full_name,
                                                                          LastAttended: address.last_attendance_date,
                                                                          AttendedInPeriod: prods.size,
                                                                          TotalAttended: all_prods.size,
                                                                          CompaniesAttendedInPeriod: requested_prods.map do |p|
                                                                            p.theater_id
                                                                          end.uniq.size,
                                                                          TotalCompaniesAttended: all_prods.map do |p|
                                                                            p.theater_id
                                                                          end.uniq.size,
