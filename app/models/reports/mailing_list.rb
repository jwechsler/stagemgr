...
class MailingList < Report
  TRG_IMPORT_HEADERS = %i[Segment Season Title FirstName LastName FullName CompanyName Email Address1 Address2
  ...
  def extract_addresses_from_ticket_orders(orders, allow_email_export = false, email_attendees = nil)
    ...
    order_set.each do |order|
      ...
      buyer_type = if order.paid_with_membership?
                     'MEM'
                   elsif order.all_tickets_complimentary?
                     'CMP'
                   else
                     'STB'
                   end
      add_hash_to_data(consolidation_code, order.address, order.performance.production, buyer_type,
  ...
  def extract_production_attendees(production, allow_email_export = false, email_attendees = nil)
    ...
    attendees.each do |address|
      ...
      add_hash_to_data(consolidation_code, address, production, consolidation_code, production.closing_at + 1.day,
  ...
  def add_hash_to_data(consolidation_code, address, production, buyer_type, performance_date, allow_email_export,
    ...
    return if @processed_addresses[production.id].include?(address.id)
    ...
    data[consolidation_code] << theater_hash.dup
    theater_hash[:Season] = ''
    data[consolidation_code] << theater_hash
    data[buyer_type] << hash unless buyer_type.nil?
