class CustomerMailingList < MailingList
  ...
  def initialize(minimum_attended = 0, minimum_revenue = 0.0, start_date = Date.current, required_theater_ids = [],
  ...
  def create
    addresses = Address.joins(orders: :performance).references(order: :performance).where(
      ...
      attended_statuses: Order::ATTENDING_STATUSES, start_date: start_date, end_date: Date.current
    ).distinct
    ...
    file_name = "/tmp/#{Admin::ReportsHelper.safe_title("customer_list#{reporting_user_id}")}.csv"
