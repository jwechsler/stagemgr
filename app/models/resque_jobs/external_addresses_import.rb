...
class ExternalAddressesImport
    ...
    CSV.foreach(filestore.path) do |row|
      if headers.nil?
        ...
        unless row[last_name_idx].blank? && row[full_name_idx].blank?
          a = Address.new
          a = ExternalAddressesImport.build_name(a, row[full_name_idx], row[first_name_idx], row[middle_name_idx],
                                                 row[last_name_idx])
          ...
          if row[external_id_idx].present?
          ...
          unless row[full_name2_idx].blank? && row[last_name2_idx].blank?
            a2 = a.dup
            a2 = ExternalAddressesImport.build_name(a2, row[full_name2_idx], row[first_name2_idx],
                                                    row[middle_name2_idx], row[last_name2_idx])
            a.address_tags.each do |tag|
  ...
  def self.build_name(address, full_name, first_name, middle_name, last_name)
    ...
    if full_name.blank?
      address.full_name = address.first_name if address.first_name.present?
      if address.middle_name.present?
        address.full_name += address.full_name.blank? ? " #{address.middle_name}" : address.middle_name
      end
      if address.last_name.present?
        address.full_name += address.full_name.blank? ? address.last_name : " #{address.last_name}"
      end
    else
