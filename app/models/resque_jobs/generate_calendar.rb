    ...
    cal = RiCal.Calendar do |cal|
      ...
      Performance.joins(:production).references(:production).where(
        ...
        unless perf.performance_date < Date.today
          desc + "\n\nFor tickets, visit #{$SERVER_CONFIG['secure_root_url']}#{Rails.application.routes.url_helpers.new_production_performance_order_path(
        ...
        cal.event do |event|
          ...
          unless perf.performance_date < Date.today
            event.url = "#{$SERVER_CONFIG['secure_root_url']}#{Rails.application.routes.url_helpers.new_production_performance_order_path(
              production_id: perf.production_id, performance_id: perf.id
            )}"
          end
        end
