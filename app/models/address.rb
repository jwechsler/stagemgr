...
class Address < ApplicationRecord
  ...
  def parse_full_name
    _, f_name, l_name = Address.parse_name(full_name)
    [f_name, l_name]
  end
  ...
  def regularize!
    ...
    line1.strip! unless line1.nil?
    self.city = city.titlecase.strip unless city.nil?
    ...
    street_number.upcase! unless street_number.nil?
    street.upcase! unless street.nil?
    street_type.upcase! unless street_type.nil?
    unit_prefix.upcase! unless unit_prefix.nil?
  ...
  def merge_and_purge(from_address)
    ...
    Address.transaction do
      update_from(from_address)
      from_address.orders.update_all(address_id: id)
  ...
  def customer_tag(order = nil)
    attendance_code = revenue_collected(18.months.ago).truncate.to_s.reverse.rjust(4, '0')
    attendance_code += performances_attended(18.months.ago).size.to_s.reverse.rjust(2, '0')
    attendance_code += "A" if is_donor?
    attendance_code += "M" if is_current_member?
    attendance_code
  end
  ...
  def active_memberships
    orders.select { |o| (o.is_a? MembershipOrder) && !o.membership.nil? && (o.membership.status == Membership::ACTIVE) }.map { |o| o.membership }
  end

  def current_membership
    active_memberships.first
  end
  ...
  def is_current_flex_pass_holder?
    orders.select { |o| (o.is_a? FlexPassOrder) && o.flex_pass_line_item.flex_pass.available? }.count > 0
  end

  def has_flex_pass?
    !FlexPassLineItem.joins(:order, :flex_passes).references(:order, :flex_passes).where(
      "orders.address_id = :address_id AND flex_passes.active = :active", address_id: id, active: true
    ).empty?
  end
  ...
  def flex_pass_candidate?
    if !has_flex_pass?
      recent_orders = orders.select { |o| o.created_at > 1.year.ago }
  ...
  def is_donor?
    donated_this_year? || donated_last_year?
  end

  def most_recent_donation_tier
    donor_tier_for_current_fiscal_year.presence || donor_tier_for_last_fiscal_year
  end

  def donated_year
    donor_tier_updated_on.year
  end
  ...
  def set_search_name
    self.search_name = full_name.gsub(/[\d+\s+.!,]/, '').upcase
    self.last_first_name = "#{last_name}#{first_name}#{middle_name}".gsub(/[\d+\s+.!,]/, '').upcase
  end
