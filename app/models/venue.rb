class Venue < ActiveRecord::Base
  ...
  def now_playing_or_next_up(production_class)
    ...
    if prods.empty? then
      future_prods = self.productions.select{|p| p.first_preview_at > Date.today && p.is_visible? && p.production_class == production_class}.sort { |p1,p2| p1.first_preview_at <=> p2.first_preview_at}
