class Venue < ApplicationRecord
  ...
  def now_playing_or_next_up(production_class, through = nil)
    ...
    if prods.empty? then
      future_prods = self.productions.select{|p| p.first_playing_date > Date.today && p.visible? && p.production_class == production_class}.sort { |p1,p2| p1.first_preview_at <=> p2.first_preview_at}
