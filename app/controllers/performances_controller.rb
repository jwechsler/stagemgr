class PerformancesController < ApplicationController
  ...
  def index
    if !@production.nil?
      ...
      @performances = @production.performances.find(:all, :conditions=>['performances.status in (?) and performances.performance_date >= ? and performances.performance_date <= ?', Performance.visible_statuses, @start_date, @end_date], :order=>'performances.performance_date, performances.performance_time asc')
