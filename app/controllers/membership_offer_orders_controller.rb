class MembershipOfferOrdersController < ApplicationController
  ...
  def new
    ...
    if @order.membership_offer.nil? || !@order.membership_offer.on_sale_to_public?
      render '/general/unavailable', layout: Rails.configuration.x.server_config['ext_site_wrapper']
      return
    end
    render '/membership_orders/edit', layout: Rails.configuration.x.server_config['ext_site_wrapper']
  end
end
