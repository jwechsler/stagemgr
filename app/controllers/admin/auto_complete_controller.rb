class Admin::AutoCompleteController < Admin::ApplicationController
  ...
  def production_code
    productions = Production.with_permissions_to(:read).where([
                                                                "LOWER(production_code) LIKE ? and status != 'Inactive'", '%' + params[:q].to_s.downcase + '%'
                                                              ])
  ...
  def any_production_code
    productions = Production.with_permissions_to(:read).where(['LOWER(production_code) LIKE ?',
