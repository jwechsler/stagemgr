class Admin::ImportsController < Admin::ApplicationController
  ...
  def create
    ...
    @production = Production.find(params[:production][:production_id]) if params[:production][:production_id].present?
  ...
  def donation_levels
    ...
    if @donation_levels_import.save
