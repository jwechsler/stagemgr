module TicketOrdersHelper
  ...
  def create_ticket_order_for_performance(performance)
    available_ticket_classes = performance.ticket_class_allocations.select { |tca|
      tca.available
    }.map { |tca| tca.ticket_class }.select { |tc| tc.web_visible unless tc.nil? }
  ...
  def preset_line_items_for_display(order)
    ...
    available = order.performance.ticket_class_allocations.select { |tca|
      !tca.ticket_class.nil? && tca.available? && !tcs.include?(tca.ticket_class.id) && tca.ticket_class.web_visible?
  ...
  def ticket_order_common_params
    common_params + [:production_code, :performance_code, :special_request,
                     ticket_line_items_attributes: [:id, :ticket_class, :ticket_class_id, :ticket_class_code, :ticket_count, :price_override, :seat_assignment_id, :_destroy]]
  end
end
