...
- **Virtual-only or Other-only orders**, such as streaming tickets or drink vouchers, are not sent to the printer. They are marked **Fulfilled** as part of the same print run, so they don't stay in Processed status.
- **Mixed orders** print only their In person tickets. Virtual and Other purchases still appear on the printed receipt.
