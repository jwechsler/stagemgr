...
from (
  ...
  (select performance_id, sum(gross_amount) gross_amount, sum(finance_fee) finance_fee from order_payments
          where date_format(convert_tz(created_at,'UTC','SYSTEM'),'%Y-%m-%d') = adddate(curdate(), interval -1 day)
          group by performance_id) perf_payments,
  performances perf,
  productions prod,
  theaters t
