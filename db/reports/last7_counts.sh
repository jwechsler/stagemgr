...
from (
  select o.id, o.performance_id, li.ticket_class_id,
         sum(ticket_count) num_sold, pay.amt amt_paid,
         round(sum(tc.ticketing_fee * li.ticket_count),2) facility_fee
  ...
  where o.status in ('Fulfilled','Processed') and li.ticket_count > 0 and li.type = 'TicketLineItem'
        and pay.order_id = o.id and
        convert_tz(o.created_at, 'UTC', 'SYSTEM') > curdate()
  group by o.order_id, ticket_class_id) order_items,
  ticket_classes,
  performances perf,
  productions prod
where
...
select date_format(sold_on, '%Y-%m-%d') sold_on, prod.name,
       count(*) orders, sum(num_sold) num_sold, format(sum(amt_paid),2) Amount
from (
...
order by prod.name, perf.performance_date desc;