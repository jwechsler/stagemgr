...
echo "
select date_format(orders.created_at, '%Y-%m-%d') sold_on, LEFT(prod.name,24) name,
...
from (
  ...
  where 
        ...
        date_format(convert_tz(o.created_at,'UTC','SYSTEM'),'%Y-%m-%d') = curdate() - interval 1 day
...
where
  ...
  perf.performance_code like '%' and
  ...
  date_format(convert_tz(orders.created_at,'UTC','SYSTEM'),'%Y-%m-%d') = curdate() - interval 1 day
group by prod.id, date_format(convert_tz(orders.created_at,'UTC','SYSTEM'), '%Y-%m-%d')
order by prod.name, perf.performance_date desc;" | /usr/local/bin/mysql --defaults-extra-file=/Users/jeremyw/.my.cnf --table=true -u stagemgr_prod stagemgr > /tmp/yesterdays_counts$1.txt
