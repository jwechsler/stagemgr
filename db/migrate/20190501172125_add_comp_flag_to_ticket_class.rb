class AddCompFlagToTicketClass < ActiveRecord::Migration
  def up
    ...
    DefaultTicketClass.where('class_code in (?)',['COMP','WITCOMP']).each do |tc|
      tc.complimentary = true
      unless tc.save
        puts "WARNING: Ticket Class #{tc.class_code} could not be updated"
      end
    end
    TicketClass.where('class_code in (?)',['COMP','WITCOMP']).each do |tc|
      tc.complimentary = true
      unless tc.save
