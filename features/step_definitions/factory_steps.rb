...
Given(/^a performance "(.*?)" exists$/) do |perf_code|
  @performance = FactoryBot.create(:performance, performance_code: perf_code,
                                                 production: Production.find_by_production_code('TEST'))
end
...
Then(/^a membership exists with status "(.*?)"$/) do |status|
  ...
  if membership.nil?
    raise "No such membership \"#{status}\" found.  Found only #{Membership.all.map do |m|
      m.status
    end.join(',')}"
  end
end
...
Then(/^a membership exists with current status "(.*?)"$/) do |status|
  ...
  raise "No membership with current status \"#{status}\" found" unless Membership.all.select do |m|
...
Then(/^a membership order exists with a gift recipient "(.*?)"$/) do |name|
  ...
  if order.nil?
    raise "No membership with gift recipient \"#{name}\" found. Only found #{Address.all.map do |a|
      a.full_name
    end}"
  end
end

Then(/^a special offer with code "(.*?)" for (\d+)% off is found$/) do |code, _percent|
...
Given(/^a ticket order for performance "(.*?)" paid with flex pass "(.*?)" exists$/) do |perf_code, pass_code|
  ...
  @ticket_order = FactoryBot.create(:ticket_order, :for_a_pair_of_tickets, :paid_with_flex_pass,
                                    flex_pass_code: pass_code, performance: perf)
end
