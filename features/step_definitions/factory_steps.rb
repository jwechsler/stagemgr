...
Then /^a membership exists with status "(.*?)"$/ do |status|
  ...
  raise "No such membership \"#{status}\" found.  Found only #{Membership.all.map { |m|
    m.status
  }.join(',')}" if membership.nil?
end

Then /^a membership exists with current status "(.*?)"$/ do |status|
  count = Membership.count
  raise "More than one membership found" if count > 1
  raise "No membership with current status \"#{status}\" found" unless Membership.all.select { |m|
...
Then /^a membership order exists with a gift recipient "(.*?)"$/ do |name|
  ...
  raise "No membership with gift recipient \"#{name}\" found. Only found #{Address.all.map { |a|
    a.full_name
  }}" if order.nil?;
end
