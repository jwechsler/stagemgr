def enter_patron_information
  ...
  if @using_admin_interface
    ...
    fill_in 'Phone', with: '555-555-1212' if page.has_field?('Phone', disabled: false)
    ...
    if page.has_css?('fieldset.fieldset legend', text: 'Billing Address')
      within(find('fieldset.fieldset legend', text: 'Billing Address').find(:xpath, '..')) do
        fill_in 'Street', with: '1229 W Belmont' if page.has_field?('Street', disabled: false)
        fill_in 'City', with: 'Chicago' if page.has_field?('City', disabled: false)
        fill_in 'State', with: 'IL' if page.has_field?('State', disabled: false)
        fill_in 'Zip/Postal', with: '60657' if page.has_field?('Zip/Postal', disabled: false)
      end
    end
  else
...
Given(/^I enter flex pass( code)? "(.*?)" as payment$/) do |_ignore, pass_code|
  @_current_form = 'ticket_order' if @_current_form.blank?
  if @using_admin_interface
