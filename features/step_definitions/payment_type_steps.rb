Given(/^I allow ([^\s]*) payments for the public$/) do |_payment_type|
...
Given(/^I disallow ([^\s]*) payments for the public$/) do |_payment_type|
  uncheck 'payment_type_allow_for_public'
end

Given(/^an external payment type "([^"]*?)" restricted to ticket classes starting with "([^"]*?)" exists$/) do |external_payment_name, _restrict_to|
  FactoryBot.create(:external_payment_type, display_name: external_payment_name, allow_for_public: false,
                                            allow_for_box_office: true, restrict_to_ticket_classes: 'CHEAP')
end

Given(/^an external payment type "([^"]*?)" exists$/) do |external_payment_name|
  FactoryBot.create(:external_payment_type, display_name: external_payment_name, allow_for_public: false,
                                            allow_for_box_office: true)
end

Given(/^I suppress the "(.*?)" method for "(.*?)"$/) do |method_name, task_type|
  click_link('add suppression')
  find('.new_order_task_suppression_task_type select', visible: :all).find(:option, task_type,
                                                                           visible: :all).select_option
  find('.new_order_task_suppression_task_method select', visible: :all).find(:option, method_name,
                                                                             visible: :all).select_option
end
