module NavigationHelpers
  ...
  def path_to(page_name)
    ...
    when /^the admin edit page for user ["'](.*)['"]$/
      @using_admin_interface = true
      url_for(controller: 'admin/users', action: 'edit', id: User.find_by_email(::Regexp.last_match(1)),
              only_path: true)
    when /^the admin theater edit page for production "([^"]*)"$/
      @using_admin_interface = true
      url_for(controller: 'admin/theaters', action: 'edit', id: Production.find_by_name(::Regexp.last_match(1)).theater.id,
              only_path: true)
    when /^the admin theater detail page for production "([^"]*)"$/
      @using_admin_interface = true
      admin_theater_path(Production.find_by_name(::Regexp.last_match(1)).theater)
    when /^the admin production detail page for "([^"]*)"$/
      @using_admin_interface = true
      production = Production.find_by_name(::Regexp.last_match(1))
    ...
    when /^the admin performance detail page for "([^"]*)"$/
      @using_admin_interface = true
      performance = Performance.find_by_performance_code(::Regexp.last_match(1))
    ...
    when /^the admin performance edit page for production "([^"]*)" and performance "([^"]*)"$/
      @using_admin_interface = true
      production = Production.find_by_name(::Regexp.last_match(1))
      performance = production.performances.select { |perf| perf.performance_code == ::Regexp.last_match(2) }.first
    ...
    when /^new web order for production "([^"]*)" and performance "([^"]*)"/
      new_production_performance_order_path(Production.find_by_name(::Regexp.last_match(1)).id,
                                            Performance.find_by_performance_code(::Regexp.last_match(2)).id)
    when /^the box office calendar for production "([^"]*)"/
      production_performances_path(Production.find_by_name(::Regexp.last_match(1)))
    when /^(the )?new membership order for membership offer "([^"]*)"/
      @_current_form = 'membership_order'
      new_membership_offer_order_path(MembershipOffer.find_by_name(::Regexp.last_match(2)).id)
    when /^the admin edit page for membership offer "([^"]*)"/
      @using_admin_interface = true
      edit_admin_membership_offer_path(MembershipOffer.find_by_name(::Regexp.last_match(1)))
    ...
    when /^the edit admin flex pass offer page for "([^"]*)"$/
      @using_admin_interface = true
      edit_admin_flex_pass_offer_path(FlexPassOffer.find_by(name: ::Regexp.last_match(1)))
    when /^the new admin flex pass order page for "(.*)"$/
      @_current_form = 'flex_pass_order'
      @using_admin_interface = true
      new_admin_flex_pass_offer_order_path(FlexPassOffer.find_by(name: ::Regexp.last_match(1)))
    ...
    when /^the address page for "([^"]*)"$/
      @using_admin_interface = true
      admin_address_path(Address.find_by_full_name(::Regexp.last_match(1)))
    when /^the edit address page for "([^"]*)"$/
      @using_admin_interface = true
      edit_admin_address_path(Address.find_by_full_name(::Regexp.last_match(1)))
    when /^the edit page for payment type "([^"]*)"$/
      @using_admin_interface = true
      edit_admin_payment_type_path(PaymentType.find_by_display_name(::Regexp.last_match(1)))
    ...
    when /^the edit page for special offer ['"](.*?)['"]$/
      @using_admin_interface = true
      edit_admin_special_offer_path(SpecialOffer.find_by_code(::Regexp.last_match(1)))
